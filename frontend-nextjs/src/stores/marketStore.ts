...
const useMarketStore = create<MarketState>()(
  subscribeWithSelector((set, get) => ({
    ...
    acknowledgeAlert: (alertId: string) => {
      set((state) => ({
        alerts: state.alerts.map(alert =>
          alert.id === alertId
            ? { ...alert, acknowledged: true } as any
            : alert
        )
      }));
    },
    ...
    fetchStockData: async (symbol: string) => {
      ...
      try {
        const response = await fetch(`http://localhost:8000/api/v1/stocks/${symbol}/price`);
    ...
    fetchMarketData: async () => {
      ...
      try {
        const [indicesResponse, sentimentResponse] = await Promise.all([
          fetch('http://localhost:8000/api/v1/market/indices'),
          fetch('http://localhost:8000/api/v1/market/sentiment')
        ]);
